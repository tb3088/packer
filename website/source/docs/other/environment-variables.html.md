...
-   `PACKER_TMP_DIR` - The directory used for temporary files during marshalling.
    If unset, appends the word 'packer' to environment variables `TEMP`, `TMP`, or
    `LOCALAPPDATA` (Windows) before falling back to the value of `configDir()/tmp`
    which resolves to (Unix: `$HOME/.packer.d/`,  Windows: `%APPDATA%/packer.d`).
